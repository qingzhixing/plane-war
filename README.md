...
竖屏弹幕射击小品：**波次清场 → 三选一强化 → Boss**，以 **评分 / 连击 / DPS** 为核心反馈，采用 **2 条命离散生命值系统**（受击扣命，生命归零时立刻结算本局）。  
...
- **惩罚**：玩家拥有固定 **2 条命**；受击在结算护盾后若仍命中则扣 1 命并短暂无敌，生命未归零时可继续战斗但会 **断连击 / 衰减评分加成**，当生命归零（Life = 0）时本局立刻进入结算；整体鼓励多打刷高分  
