...
- **玩家护盾特效**
  - 表现：当玩家当前存在护盾（或“可挡一次伤害”的保护状态）时，玩家机体外侧出现一圈半透明护盾光圈。
  - 命中反馈：
    - 若护盾吸收一次伤害：护盾立即强烈闪光（加亮 + 放大 + 轻微抖动），随后消失。
    - 若护盾仍然存在（例如堆叠护盾的后续设计），则只是短暂闪光，不完全消失。
  - 技术实现：
    - 使用独立的 `Shield` 精灵或 `Sprite2D` + Shader，跟随玩家位置。
    - Shader 至少支持：颜色渐变（从蓝/青色到白色高光）、外缘柔和扩散、轻微扭曲效果。
    - 护盾是否可见由玩家脚本控制（例如 `has_shield` 布尔状态）。
