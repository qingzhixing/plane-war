...
func _load_from_json() -> void:
	...
	if file == null:
		_LogBridgeRef.warn("BattleProgressionConfig missing file: %s" % _CONFIG_JSON_PATH)
		return
	var parsed: Variant = JSON.parse_string(file.get_as_text())
	if typeof(parsed) != TYPE_DICTIONARY:
		_LogBridgeRef.error("BattleProgressionConfig parse failed: root is not dictionary")
		return
