...
func apply_main_upgrade(main: Node, upgrade_id: String) -> bool:
	...
	before_payload = _ModExtensionBridgeRef.dispatch_event("before_apply_main_upgrade", before_payload)
	...
	var applied := _ModExtensionBridgeRef.try_apply_main_upgrade_effect(main, resolved_upgrade_id)
	_ModExtensionBridgeRef.dispatch_event(
		"after_apply_main_upgrade",
		{
			"main": main,
